...
class C(nn.Module):
    def __init__(self, G, delta, k):
        ...
        for node in self.G.nodes():
            ...
            count += 1
        self.endowments = nn.Embedding(self.N, k).requires_grad_(True) #using nn.Embedding

        self.delta = torch.nn.Parameter(torch.FloatTensor(1), requires_grad=False)
        self.delta.data.fill_(delta)
    ...
    def forward(self, x): # input a node of the Graph
        ...
        for j in self.G.neighbors(x): # find utility for all neighbors of this node
            ...
            for h in list(nx.common_neighbors(self.G, x, j)):
                u += self.delta[0] * torch.sum(torch.relu((self.endowments(self.lookup[h]) - self.endowments(self.lookup[x])).squeeze()))
            self.c[x][self.lookup[j][0]] = u # insert the utility
        self.c[x] = torch.softmax(self.c[x], 0) # softmax
        return self.c[x]

def model_run(delta, dimension):
    n_sample = 1000
    G = nx.read_gexf('Graph/Season/Duration/G_Duration_Season_1.gexf')
    G2 = nx.read_gexf('Graph/Season/Duration/G_Duration_Season_1.gexf')
    Lookup = dict()
    count = 0
    for node in G.nodes():
        Lookup[node] = count
    ...
    model = C(G=G, delta=delta, k=dimension)
    ...
    for epoch in range(epochs):
        ...
        if epoch%100 == 99:
            ...
            df = pd.DataFrame(H)
            df.to_csv('endowments_{}_{}.csv'.format(delta, dimension), index=False, header=False)


def Evaluation():
    G = nx.read_gexf('Graph/Season/Duration/G_Duration_Season_3.gexf')
    Edges = list(G.edges()).copy()
    endowments = read_endowments('endowments_0.2_4.csv')
    def Cij():
        C = {}
        for node in G.nodes():
            C[node] = {}
            for y in nx.neighbors(G, node):
                ...
                for cn in nx.common_neighbors(G, node, y):
                    u += 0.2 * np.sum(np.maximum(np.array(endowments[cn] - np.array((endowments[node]))), 0)) # 0.0297
                C[node][y] = u
...
def Direct_Effect():
    ...
    plt.ylim([0,0.3])
    plt.xlabel("Tie Range", fontsize=25)
    plt.ylabel("Average Direct Effect", fontsize=25)
    plt.show()
...
def Indirect_Effect():
    ...
    for edge in Edges:
        ...
        if cn > 0:
            if cn >= 20:
                H[19].append(C[edge[0]][edge[1]])
                H[19].append(C[edge[1]][edge[0]])
            else:
                H[cn - 1].append(C[edge[0]][edge[1]])
                H[cn - 1].append(C[edge[1]][edge[0]])
    ...
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    plt.xticks([0, 5, 10, 15, 20],['0','5', '10', '15','$\geq20$'], fontsize=20)
    plt.yticks(fontsize=20)
    plt.ylim([0, 0.3])
    plt.xlabel("Common Neighbors", fontsize=25)
    plt.ylabel("Average Indirect Effect", fontsize=25)
    plt.show()
...
def LearningCurve():
    ...
    loss_des = torch.nn.MSELoss(reduction='sum')
    t0 = time.time()
    Sampled = np.random.choice(Nodes, 1000, p=p.ravel())
    indices = torch.tensor([Lookup[node] for node in Sampled])
    ADJ_test = torch.index_select(A, 0, indices)
    for epoch in range(epochs):
        random_node = np.random.choice(Nodes, n_sample, p=p.ravel())
        indices = torch.tensor([Lookup[node] for node in random_node])
        ADJ = torch.index_select(A, 0, indices)
        ...
        ADJ_hat_test = []
        for node in Sampled:
            ADJ_hat_test.append(model(node).unsqueeze(0))
        ADJ_hat_test = torch.cat(ADJ_hat_test)
        loss = loss_fn(ADJ_hat, ADJ)
        writer.add_scalar("Training_loss", loss/n_sample, epoch)
        L = loss_des(ADJ_hat_test, ADJ_test)/len(Sampled)
        writer.add_scalar("Testing_loss", L, epoch)
        print(epoch, time.time() - t0, loss/n_sample, L)
...
def Comparison(delta, dimension):
    ...
    for i in range(len(A)):
        A[i] = np.squeeze(np.log(A[i] + 1))
    for node in G.nodes():
        interactions[node] = np.sum(A[Lookup[node]])
    for i in range(len(A)):
        if np.sum(A[i]) != 0:
            A[i] = A[i] / np.sum(A[i])
    ...
    endowments = read_endowments('endowments_{}_{}.csv'.format(delta, dimension))
    ...
    for edge in Edges:
        ...
        if tr < 100:
            if tr >= 6:
                H[4].append(C[edge[0]][edge[1]])
                H[4].append(C[edge[1]][edge[0]])
                H2[4].append(A[Lookup[edge[0]]][Lookup[edge[1]]])
                H2[4].append(A[Lookup[edge[1]]][Lookup[edge[0]]])
            else:
                H[tr - 2].append(C[edge[0]][edge[1]])
                H[tr - 2].append(C[edge[1]][edge[0]])
                H2[tr - 2].append(A[Lookup[edge[0]]][Lookup[edge[1]]])
                H2[tr - 2].append(A[Lookup[edge[1]]][Lookup[edge[0]]])
    ...
    for i in range(5):
        Y2.append(np.mean(H2[i]))
        Err2.append(mean_confidence_interval(H2[i]))
    plt.errorbar(x=X, y=Y2, yerr=Err2, fmt="o", color="#2980b9", ls='--', label='Real',
                 ecolor='#2980b9', marker='8', markersize=10, linewidth=2, elinewidth=0.5, capsize=4)
    ...
    plt.xlabel("Tie Range", fontsize=25)
    plt.ylabel("Average Benefits", fontsize=25)
    legend = plt.legend(frameon=False, loc='upper right', title='Results', fontsize=20)
    legend.get_title().set_fontsize(fontsize=20)
    plt.ylim([0, 0.3])
    plt.title('delta={}, dimension=5{}'.format(delta, dimension), fontsize=30)
    plt.show()